...
describe("opt-in insecure automatic fallback", () => {
  ...
  test("automatic fallback reason and level remain immutable after boot", async () => {
    const fixture = makeFixture();
    const result = await boot(fixture, {
      floor: "0",
      auto: "1",
      t0: { available: false, reason: "initial t0 failure" },
      t1: { available: true, reason: null },
    });
    expect(result.code).toBe(0);
    ...
    const all = readRecords(fixture.records);
    expect(all.filter((entry) => entry.event === "probe")).toHaveLength(1);
    expect(all.find((entry) => entry.event === "factory" && entry.kind === "Tier0")?.level).toBe(2);
    expect(result.stderr).toContain("WARN: FACET_INSECURE=2 — auto:initial t0 failure");
  });
