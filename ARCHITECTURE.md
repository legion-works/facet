...
The gallery uses opaque-origin iframe frames with `sandbox="allow-scripts"` and a frozen CSP. Two MessageChannels separate one-shot source ingress from closure-held control. Every revision receives a fresh frame. Double-buffered hot swap preserves shell view state while replacing the artifact frame. Interactive `tsx` artifacts execute inside an isolated nested frame with a per-frame CSP nonce (`src/gallery-web/frame/renderers/tsx.ts`). Shell session state is persisted to `sessionStorage` (`src/gallery-web/session.ts`) so tab refreshes survive without re-submitting the single-use bootstrap token.
